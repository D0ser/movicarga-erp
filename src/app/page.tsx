...
export default function Login() {
	...
	const [loginBlockInfo, setLoginBlockInfo] = useState<{ blocked: boolean; remainingMinutes?: number; attemptsLeft?: number } | null>(null);
	...
	const handleLogin = async (e: React.FormEvent) => {
		...
		try {
			...
			if (blockStatus.blocked) {
				...
				setError(`Cuenta bloqueada temporalmente. Intente nuevamente en ${blockStatus.remainingMinutes} minutos.`);
				setIsValidating(false);
				return;
			}
			...
			const hardcodedUser = users.find((user) => user.username === username && user.password === password);
			...
			const { data: usuariosData, error: usuariosError } = await supabase.from("usuarios").select("*").eq("nombre", nombre).eq("apellido", apellido).eq("estado", true);
			...
			if (usuariosData && usuariosData.length > 0) {
				...
				await supabase.from("usuarios").update({ ultimo_acceso: new Date().toISOString() }).eq("id", userFromDB.id);
		...
		<div className={styles.loginContainer}>
			<CustomCard className="max-w-md w-full p-6">
				...
				<CardContent>
					...
					{loginBlockInfo && !loginBlockInfo.blocked && loginBlockInfo.attemptsLeft && loginBlockInfo.attemptsLeft <= 2 && (
						<CustomAlert variant="warning" className="mb-4">
							Atención: Le quedan {loginBlockInfo.attemptsLeft} intentos antes de que su cuenta sea bloqueada temporalmente.
						</CustomAlert>
					)}
						...
						{!showTwoFactorInput ? (
							...
							<div className="space-y-2">
								...
								<Input
									...
									maxLength={6}
								/>
								<p className="text-xs text-gray-500 text-center">Ingrese el código de verificación generado por su aplicación de autenticación.</p>
							</div>
						)}
