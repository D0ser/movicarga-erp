...
export default function EgresosPage() {
	...
	const columns: Column<Egreso>[] = [
			...
			cell: (value) => {
				...
				return (
					<div className="flex items-center px-2 justify-center">
						<div className="flex-shrink-0 h-7 w-7 rounded-full bg-cyan-100 text-cyan-800 flex items-center justify-center font-bold mr-2">{inicial}</div>
		...
		},
		{
			...
			cell: (value) => (
				<div className="flex justify-center">
					{(value as string) !== "N/A" ? <span className="font-mono bg-gray-50 px-2 py-1 rounded text-gray-700 text-sm">{value as string}</span> : <span className="text-gray-400">-</span>}
				</div>
			),
		},
		{
			...
			cell: (value) => {
				...
				return (
					<div className="flex justify-center">
						<span className={`px-2 py-1 rounded-full text-xs font-medium ${bgColor} ${textColor}`}>{tipoEgreso}</span>
					</div>
				);
			},
		},
		...
		},
		{
			...
			cell: (value) => {
				...
				return (
					<div className="flex justify-center">
						<span className={`px-2 py-1 rounded-full text-xs font-medium flex items-center ${aprobado ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}>
							{aprobado ? (
								<>
									<svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
