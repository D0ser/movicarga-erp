...
export default function ListaEgresosPage() {
  ...
  const [tablaActiva, setTablaActiva] = useState<
    'cuentas_banco' | 'empresas' | 'tipos_egreso' | 'tipos_egreso_sf'
  >('cuentas_banco');
      ...
      if (tablaActiva === 'cuentas_banco') {
        ...
        try {
          ...
          notificationService.error(
            'No se pudieron cargar las cuentas de banco. Inténtelo de nuevo más tarde.'
          );
        } finally {
          setLoading(false);
        }
    ...
    async function cargarEmpresas() {
      if (tablaActiva === 'empresas') {
        ...
        try {
          ...
          notificationService.error(
            'No se pudieron cargar las empresas. Inténtelo de nuevo más tarde.'
          );
        } finally {
          setLoading(false);
        }
    ...
    async function cargarTiposEgreso() {
      if (tablaActiva === 'tipos_egreso') {
        ...
        try {
          ...
          notificationService.error(
            'No se pudieron cargar los tipos de egreso. Inténtelo de nuevo más tarde.'
          );
        } finally {
          setLoading(false);
        }
    ...
    async function cargarTiposEgresoSF() {
      if (tablaActiva === 'tipos_egreso_sf') {
        ...
        try {
          ...
          notificationService.error(
            'No se pudieron cargar los tipos de egreso sin factura. Inténtelo de nuevo más tarde.'
          );
        } finally {
          setLoading(false);
        }
      ...
      cell: (value: unknown) => {
        ...
        return (
          <div className="text-center">
            {moneda === 'Soles' ? 'S/.' : moneda === 'Dólares' ? '$' : moneda}
          </div>
        );
      },
    },
  ...
  const handleInputChangeCuentasBanco = (
  ...
  const handleSubmitCuentasBanco = async (e: React.FormEvent) => {
    ...
    try {
      const cuentaBancoDatos = {
        ...
        fecha_creacion:
          formDataCuentasBanco.fecha_creacion || new Date().toISOString().split('T')[0],
      };

      if (formDataCuentasBanco.id) {
        // Actualizar cuenta existente
        await cuentaBancoService.updateCuentaBanco(
          formDataCuentasBanco.id as string,
          cuentaBancoDatos
        );
      ...
      notificationService.error(
        'No se pudo guardar la cuenta bancaria. Inténtelo de nuevo más tarde.'
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleDeleteCuentaBanco = async (id: string) => {
    try {
      ...
      const confirmed = await cuentaBancoConfirm.confirm();
      ...
      notificationService.error(
        'No se pudo eliminar la cuenta bancaria. Inténtelo de nuevo más tarde.'
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleDeleteEmpresa = async (id: string) => {
    try {
      // Esperar confirmación mediante el diálogo
      const confirmed = await empresaConfirm.confirm();
  ...
  const handleSubmitTiposEgreso = async (e: React.FormEvent) => {
    ...
    try {
      const tipoEgresoDatos = {
        ...
        fecha_creacion:
          formDataTiposEgreso.fecha_creacion || new Date().toISOString().split('T')[0],
      };
      ...
      notificationService.error(
        'No se pudo guardar el tipo de egreso. Inténtelo de nuevo más tarde.'
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleDeleteTipoEgreso = async (id: string) => {
    try {
      // Esperar confirmación mediante el diálogo
      const confirmed = await tipoEgresoConfirm.confirm();
      ...
      notificationService.error(
        'No se pudo eliminar el tipo de egreso. Inténtelo de nuevo más tarde.'
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmitTiposEgresoSF = async (e: React.FormEvent) => {
    ...
    try {
      const tipoEgresoSFDatos = {
        ...
        fecha_creacion:
          formDataTiposEgresoSF.fecha_creacion || new Date().toISOString().split('T')[0],
      };

      if (formDataTiposEgresoSF.id) {
        // Actualizar tipo existente
        await tipoEgresoSFService.updateTipoEgresoSF(
          formDataTiposEgresoSF.id as string,
          tipoEgresoSFDatos
        );
      ...
      notificationService.error(
        'No se pudo guardar el tipo de egreso sin factura. Inténtelo de nuevo más tarde.'
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleDeleteTipoEgresoSF = async (id: string) => {
    try {
      // Esperar confirmación mediante el diálogo
      const confirmed = await tipoEgresoSFConfirm.confirm();

      if (confirmed) {
        setLoading(true);
        await tipoEgresoSFService.deleteTipoEgresoSF(id);
        const tiposSFActualizados = await tipoEgresoSFService.getTiposEgresoSF();
        setTiposEgresoSF(tiposSFActualizados);
      ...
      notificationService.error(
        'No se pudo eliminar el tipo de egreso sin factura. Inténtelo de nuevo más tarde.'
      );
    } finally {
      setLoading(false);
    }
  ...
  const cuentaBancoConfirm = useConfirmDialog({
    title: 'Eliminar Cuenta Bancaria',
    description: '¿Está seguro de que desea eliminar esta cuenta bancaria?',
    type: 'error',
    variant: 'destructive',
    confirmText: 'Eliminar',
  });

  const empresaConfirm = useConfirmDialog({
    title: 'Eliminar Empresa',
    description: '¿Está seguro de que desea eliminar esta empresa?',
    type: 'error',
    variant: 'destructive',
    confirmText: 'Eliminar',
  });

  const tipoEgresoConfirm = useConfirmDialog({
    title: 'Eliminar Tipo de Egreso',
    description: '¿Está seguro de que desea eliminar este tipo de egreso?',
    type: 'error',
    variant: 'destructive',
    confirmText: 'Eliminar',
  });

  const tipoEgresoSFConfirm = useConfirmDialog({
    title: 'Eliminar Tipo de Egreso Sin Factura',
    description: '¿Está seguro de que desea eliminar este tipo de egreso sin factura?',
    type: 'error',
    variant: 'destructive',
    confirmText: 'Eliminar',
  });
    ...
    <div className="space-y-6">
      ...
      {tablaActiva === 'cuentas_banco' && (
        <div className="space-y-6">
          ...
          <Modal
            isOpen={showFormCuentasBanco}
            onClose={() => setShowFormCuentasBanco(false)}
            title={formDataCuentasBanco.id ? 'Editar Cuenta Bancaria' : 'Nueva Cuenta Bancaria'}
            size="md"
          >
      ...
      {tablaActiva === 'empresas' && (
        <div className="space-y-6">
          ...
          {/* Modal de Empresas */}
          <Modal
            isOpen={showFormEmpresas}
            onClose={() => setShowFormEmpresas(false)}
            title={formDataEmpresas.id ? 'Editar Empresa' : 'Nueva Empresa'}
            size="md"
          >
      ...
      {tablaActiva === 'tipos_egreso' && (
        <div className="space-y-6">
          ...
          <Modal
            isOpen={showFormTiposEgreso}
            onClose={() => setShowFormTiposEgreso(false)}
            title={formDataTiposEgreso.id ? 'Editar Tipo de Egreso' : 'Nuevo Tipo de Egreso'}
            size="md"
          >
      ...
      {tablaActiva === 'tipos_egreso_sf' && (
        <div className="space-y-6">
          ...
          <Modal
            isOpen={showFormTiposEgresoSF}
            onClose={() => setShowFormTiposEgresoSF(false)}
            title={formDataTiposEgresoSF.id ? 'Editar Tipo Egreso S/F' : 'Nuevo Tipo Egreso S/F'}
            size="md"
          >
            <form onSubmit={handleSubmitTiposEgresoSF} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700">
                  Tipo de Egreso Sin Factura
                </label>
                <input
                  type="text"
                  name="tipo"
          ...
          <DataTable
            columns={columnasTiposEgresoSF}
            data={tiposEgresoSF}
            title="Tipos de Egreso Sin Factura"
            isLoading={loading}
          />
        </div>
      )}
      ...
      {/* Diálogos de confirmación */}
      <ConfirmDialog {...cuentaBancoConfirm.dialogProps} />
      <ConfirmDialog {...empresaConfirm.dialogProps} />
      <ConfirmDialog {...tipoEgresoConfirm.dialogProps} />
      <ConfirmDialog {...tipoEgresoSFConfirm.dialogProps} />
    </div>
  );
}
